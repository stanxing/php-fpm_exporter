...
| `--web.disable-process-state-metrics` | Exclude metrics about phpfpm process state. `--phpfpm.fix-process-count` will be set to `false` by default if the flag is set to `ture`| `PHP_FPM_WEB_DISABLE_PROCESS_STATE_METRICS` | `false` |
